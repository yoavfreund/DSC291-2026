...
\begin{frame}{Fit PoissonGAM per Year}
  \begin{itemize}
...
\begin{lstlisting}
gam = PoissonGAM(s(0, n_splines=12, spline_order=3)).fit(X, y)
\end{lstlisting}
  \end{exampleblock}
\end{frame}
...
\begin{frame}{Year-to-Year Fit Summary}
  \begin{itemize}
    \item \textbf{Top:} Year-level means (aggregated)---observed and predicted can look very close even when period-level errors exist
    \item \textbf{Middle:} Period-level scatter---spread around diagonal expected for Poisson counts
    \item \textbf{Bottom:} AIC by year---lower is better
  \end{itemize}
\end{frame}
...
\begin{frame}{Summary}
  \begin{enumerate}
    ...
    \item Year-level aggregates can look good even with period-level spread
  \end{enumerate}
\end{frame}
